...
func (tt *ToolTip) addTool(hwnd win.HWND, track bool) error {
	...
	if win.FALSE == tt.SendMessage(win.TTM_ADDTOOL, 0, uintptr(unsafe.Pointer(&ti))) {
		// PATCHED (bravo-bis-configurator): TTM_ADDTOOL intermittently
		// fails on Windows 11 (unresolved upstream: lxn/walk#805), even
		// with a Common-Controls-v6 application manifest, whenever a new
		// per-thread WindowGroup (and therefore a fresh ToolTip control)
		// is created after a previous one was torn down — e.g. opening
		// the main editor window right after a modal Dialog closes.
		// Failing here previously made the *entire* window fail to
		// construct just because one widget's hover-tooltip registration
		// didn't stick. Per the community-verified workaround in that
		// issue, treat this as non-fatal: the affected widget simply
		// won't show a hover tooltip, everything else still works.
		return nil
	}
